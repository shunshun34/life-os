...
export function WorkoutV5Section() {
  const [plans,setPlans]=useState<any[]>([]); const [items,setItems]=useState<any[]>([]); const [logs,setLogs]=useState<any[]>([]); const [weekCount,setWeekCount]=useState(0); const [msg,setMsg]=useState<Msg>(null); const today=todayISO();
  async function load(){const uid=await userId(); const dow=dayOfWeek(); const ws=weekStartISO(); const [{data:p},{data:allp},{data:l},{data:w}]=await Promise.all([supabase.from("workout_plans").select("*").eq("user_id",uid).eq("day_of_week",dow),supabase.from("workout_plans").select("*").eq("user_id",uid).order("day_of_week"),supabase.from("workout_logs").select("*").eq("user_id",uid).eq("date",today),supabase.from("workout_logs").select("date").eq("user_id",uid).gte("date",ws).lte("date",today)]); const ps=allp??[]; setPlans(ps); setLogs(l??[]); setWeekCount(new Set((w??[]).map(x=>x.date)).size); const todayPlan=(p??[])[0]; if(todayPlan){const {data:i}=await supabase.from("workout_plan_items").select("*").eq("plan_id",todayPlan.id).order("display_order"); setItems(i??[]);} else setItems([]);}
  useEffect(()=>{load().catch(e=>setMsg({type:"error",text:e.message}))},[]);
  async function addPlan(fd:FormData){try{const uid=await userId(); await insert("workout_plans",{user_id:uid,name:String(fd.get("name")),day_of_week:Number(fd.get("day_of_week")),estimated_minutes:num(fd.get("estimated_minutes")),memo:String(fd.get("memo")||"")}); setMsg({type:"ok",text:"メニューを作成しました。"}); await load();}catch(e){setMsg({type:"error",text:e instanceof Error?e.message:"作成に失敗しました。"})}}
  async function addItem(fd:FormData){try{await insert("workout_plan_items",{plan_id:String(fd.get("plan_id")),exercise_name:String(fd.get("exercise_name")),default_weight:num(fd.get("default_weight")),default_reps:num(fd.get("default_reps")),default_sets:num(fd.get("default_sets")),display_order:num(fd.get("display_order"))??0}); setMsg({type:"ok",text:"種目を追加しました。"}); await load();}catch(e){setMsg({type:"error",text:e instanceof Error?e.message:"追加に失敗しました。"})}}
  async function saveLog(fd:FormData, item:any){try{const uid=await userId(); const todayPlan=plans.find(p=>p.day_of_week===dayOfWeek()); await insert("workout_logs",{user_id:uid,date:today,plan_id:todayPlan?.id,exercise_name:item.exercise_name,weight:num(fd.get("weight")),reps:num(fd.get("reps")),sets:num(fd.get("sets")),cardio_minutes:num(fd.get("cardio_minutes")),memo:String(fd.get("memo")||"")}); setMsg({type:"ok",text:"筋トレ記録を保存しました。"}); await load();}catch(e){setMsg({type:"error",text:e instanceof Error?e.message:"保存に失敗しました。"})}}
  const todayPlan=plans.find(p=>p.day_of_week===dayOfWeek());
  return <div className="space-y-6"><Message msg={msg}/><Card><div className="text-sm text-slate-500">今週の実施回数</div><div className="text-3xl font-black">{weekCount}回</div></Card><Card><H2>今日の筋トレ（{dayLabels[dayOfWeek()]}）</H2><div className="mt-3 font-bold">{todayPlan?.name ?? "未設定"}</div><div className="mt-4 space-y-4">{items.map(i=><form key={i.id} action={(fd)=>saveLog(fd,i)} className="rounded-2xl bg-slate-50 p-4"><div className="font-bold">{i.exercise_name}</div><div className="mt-3 grid gap-3 md:grid-cols-4"><Input label="重量" name="weight" type="number" defaultValue={i.default_weight}/><Input label="回数" name="reps" type="number" defaultValue={i.default_reps}/><Input label="セット" name="sets" type="number" defaultValue={i.default_sets}/><Input label="有酸素分" name="cardio_minutes" type="number"/></div><div className="mt-3"><Input label="一言ログ" name="memo"/></div><div className="mt-3"><Btn>記録</Btn></div></form>)}{items.length===0&&<div className="text-sm text-slate-500">今日の種目がありません。</div>}</div></Card><Card><H2>メニュー作成</H2><form action={addPlan} className="mt-5 grid gap-4 md:grid-cols-3"><Input label="メニュー名" name="name"/><Select label="曜日" name="day_of_week">{dayLabels.map((d,i)=><option key={i} value={i}>{d}</option>)}</Select><Input label="所要時間" name="estimated_minutes" type="number"/><Input label="メモ" name="memo"/><div className="md:col-span-3"><Btn>作成</Btn></div></form></Card><Card><H2>種目追加</H2><form action={addItem} className="mt-5 grid gap-4 md:grid-cols-3"><Select label="メニュー" name="plan_id">{plans.map(p=><option key={p.id} value={p.id}>{dayLabels[p.day_of_week]}：{p.name}</option>)}</Select><Input label="種目名" name="exercise_name"/><Input label="標準重量" name="default_weight" type="number"/><Input label="標準回数" name="default_reps" type="number"/><Input label="標準セット" name="default_sets" type="number"/><Input label="表示順" name="display_order" type="number"/><div className="md:col-span-3"><Btn>種目追加</Btn></div></form></Card></div>
}
